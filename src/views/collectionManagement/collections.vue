<template>
  <div class="main-box">
    ...
    <batch-collection v-if="openBatchDrawer" :batch-team-id="batchTeamId" :current-row="currentRow" @closeBatch="closeBatch" />
    <add-push-down v-if="openRecordDrawer" :current-row="recordRow" @closeAdd="closeRecord" />
  </div>
</template>

<script>
import { getCompanyGroup, getGroupCollectionUser } from '@/api/collectionAgentManagement'
import { getCollectionInfo, exportCollectionInfo } from '@/api/collectionManagement'
import Pagination from '@/components/Pagination' // 基于 el-pagination 的二次包
import { mapState } from 'vuex'
import batchCollection from './components/collectionOrder/batchCollection'
...
export default {
  ...
  computed: {
    ...
    statusOptions() {
      return [
        { value: 70, label: this.$t('collect.collect32') }, /* 逾期中 */
        { value: 1010, label: this.$t('collect.collect34') } /* 逾期已还款(逾期完成) */
      ]
    }
  },
  ...
  methods: {
    ...
    async getTableList(isSearch) {
      ...
      const res = await getCollectionInfo(tmp)
    ...
    async exportData() {
      ...
      const res = await exportCollectionInfo(tmp)
      ...
      fileReader.onload = (event) => {
        ...
        a.download = `collection_info_${nowStr}.xlsx`
