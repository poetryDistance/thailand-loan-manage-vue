    ...
    >
      <el-table-column :label="$t('sysManage.sysManage23')" align="center" min-width="140px"> <!-- 操作 -->
        <template slot-scope="{ row }">
          ...
          <el-button v-if="!permission.isAdmin" size="mini" type="text" @click="openRecord(row)">
...
import { inRemindOrder } from '@/api/remindController'
...
export default {
  ...
  methods: {
    ...
    async getTableList(isSearch) {
      ...
      const res = await inRemindOrder(tmp)
